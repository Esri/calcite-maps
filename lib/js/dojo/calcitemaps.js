...
define([ 
  ...
  "dojo/domReady!"
		...
		query("#mainNav .dropdown").on("show.bs.dropdown", function() {
			if (!_mobile) {
				domClass.add(query("#panelAccordion")[0], "fade-out opacity-75");
			}
		});
		query("#mainNav .dropdown").on("hide.bs.dropdown", function() {
			if (!_mobile) {
				domClass.remove(query("#panelAccordion")[0], "fade-out opacity-75");
			}
		});
		...
		query(window).on("resize", function() {
			...
			if (!_mobile && isMobile()) {
				...
				query("#panelAccordion .panel.in.panel-mobile-only").collapse("hide");
			}
			_mobile = isMobile();
		});
